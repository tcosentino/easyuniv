...
define([
		...
		dragged.fetch({ success: function() {
			...
			dropped.fetch({ success: function() {
				...
				if(dropped.get('schoolID') == 2) {
					$('ul.general')
						.append("<li class='unused-tile btn ui-draggable' data-id='" 
										+ dropped.get('id') 
										+ "' data-userCount='"
										+ dropped.get('userCount')
										+ "'>"
										+ dropped.get('text')
  ...
  var HomeSetupView = Backbone.View.extend({
						  ...
						  console.log(userSchool.toJSON());
							...
							allTiles.fetch({ success: function() {
								var schoolTiles = new HomeTiles([
								  allTiles.get(userSchool.get('tile1')),
									allTiles.get(userSchool.get('tile2')),
									allTiles.get(userSchool.get('tile3')),
									allTiles.get(userSchool.get('tile4')),
									allTiles.get(userSchool.get('tile5'))
								]);//end schoolTiles
								...
								general.fetchBySite('general', function(exists) {
									if(!exists) {
										...
										var genTiles = new HomeTiles([
											allTiles.get(general.get('tile1')),
											allTiles.get(general.get('tile2')),
											allTiles.get(general.get('tile3')),
											allTiles.get(general.get('tile4')),
											allTiles.get(general.get('tile5'))
										]);//end genTiles
										...
										var data = {
											...
											'unusedSchool': allTiles.where({schoolID: userSchool.get('id'), standard: '0'}),
											'unusedGeneral': allTiles.where({schoolID: '2', standard: '0'}),
											_: _
										};
	...
	listeners: function(){
	  ...
	  $('#right_arrow').live('click', {view: this}, this.rightArrow);
	  $('#left_arrow').live('click', this.leftArrow);
	},
